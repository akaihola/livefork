...
def draft(
    ...
    When the ``llm`` package is installed the draft is generated by an LLM
    from the branch's commit log and diff.  Install provider plugins via
    extras – e.g. ``pip install livefork[anthropic]``.
    """
    import subprocess
    ...
    # ---- generate content (LLM or template) ----
    if not draft_file.exists():
        upstream_ref = f"{cfg.upstream.remote}/{cfg.upstream.branch}"
        try:
            merge_base = git.get_merge_base(upstream_ref, branch)
        except Exception:
            merge_base = None

        generated = False
        if merge_base is not None:
            log = git.get_log_messages(f"{merge_base}..{branch}")
            diff = git.get_diff_range(merge_base, branch)
            try:
                from livefork.draft_llm import generate_draft
                ...
                typer.echo(
                    "llm package not installed – using template.  "
                    "Install it with: pip install 'livefork[anthropic]' "
                    "(or [gemini], [openai])",
                    err=True,
                )
            except Exception as exc:
                typer.echo(f"LLM generation failed: {exc}", err=True)

        if not generated:
            fallback = DraftContent(
                branch=branch,
                title=message or branch,
                body="Describe the purpose of this PR.",
            )
            draft_file.write_text(format_draft(fallback))

    # ---- apply --message title override ----
    if message and draft_file.exists():
        dc = parse_draft(draft_file.read_text())
        dc.title = message
        draft_file.write_text(format_draft(dc))
...
def pr_create(
    ...
    from livefork.draft_format import parse_draft
